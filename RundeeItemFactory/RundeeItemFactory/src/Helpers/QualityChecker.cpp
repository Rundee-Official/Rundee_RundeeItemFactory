...
namespace QualityChecker
{
    QualityResult CheckWeaponQuality(const ItemWeaponData& item)
    {
        ...
        else if (item.weaponCategory == "Melee")
        {
            ...
            // Melee weapons: fireRate must be 0 (melee weapons don't have fire rate)
            if (item.fireRate != 0)
            {
                result.errors.push_back("Melee weapons cannot have fireRate > 0 (must be 0)");
    ...
    QualityResult CheckMedicineQuality(const ItemMedicineData& item)
    {
        ...
        // Check medicine characteristics
        if (item.healthRestore < 10)
        {
            result.warnings.push_back("Medicine with very low health restore");
            result.qualityScore -= 10.0f;
        }

        if (item.healthRestore > 100)
        {
            result.warnings.push_back("Medicine with health restore exceeding 100");
            result.qualityScore -= 5.0f;
        }

        // Medicine should prioritize health restoration
        if (item.healthRestore < 20 && item.rarity == "Rare")
        {
            result.warnings.push_back("Rare medicine should have higher health restore");
            result.qualityScore -= 15.0f;
        }
