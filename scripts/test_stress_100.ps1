# 스트레스 테스트 스크립트 - 각 타입 100개씩 생성
# 정확히 100개가 생성되는지, 레지스트리 파일이 생성되는지 확인
...
Write-Host "스트레스 테스트: 각 타입 100개씩 생성" -ForegroundColor Yellow
...
foreach ($itemType in $itemTypes) {
    ...
    Write-Host "[$itemType] 100개 생성 테스트 시작" -ForegroundColor Cyan
    ...
    Write-Host "생성 중: $itemType (100개)..." -ForegroundColor Yellow
    ...
    try {
        ...
        Write-Host "  생성된 아이템 수: $actualCount" -ForegroundColor $(if ($actualCount -eq 100) { "Green" } else { "Yellow" })
        Write-Host "  소요 시간: $([math]::Round($duration, 2))초" -ForegroundColor Gray
        
        if ($actualCount -eq 100) {
            Write-Host "  PASS: 정확히 100개 생성됨" -ForegroundColor Green
            $results[$itemType] = @{
                success = $true
                count = $actualCount
                exitCode = 0
        ...
        } elseif ($actualCount -gt 100) {
            Write-Host "  FAIL: $actualCount 개 생성됨 (100개 초과)" -ForegroundColor Red
            $results[$itemType] = @{
                success = $false
                count = $actualCount
                exitCode = 0
            ...
            Write-Host "  FAIL: $actualCount 개만 생성됨 (100개 미만)" -ForegroundColor Red
            $results[$itemType] = @{
                success = $false
                count = $actualCount
                exitCode = 0
